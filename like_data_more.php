...
if(ISSET($_POST["name"])){
    ...
    $like_data_query = "SELECT `like`.company_name,company.company_point FROM galea_company_like as `like` LEFT OUTER JOIN galea_company as company ON company.company_name = `like`.company_name WHERE `like`.username = '$username' ORDER BY `like`.like_time DESC LIMIT 4 OFFSET {$now_num}";
